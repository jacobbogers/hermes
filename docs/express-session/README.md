...
Specifies the value for the `Domain` field in the `Set-Cookie` response header. By default, no domain
is set, and most clients will consider the cookie to apply to only the current
domain.

##### cookie.expires

Specifies the `Date` object to be the value for the `Expires` field in the `Set-Cookie` response header.
...
**Note** If both `expires` and `maxAge` are set in the options, then the `maxAge` will
take precedence.
...
Specifies the `boolean` value for including the `HttpOnly` field in the  `Set-Cookie` response header. When truthy, the `HttpOnly` attribute field is set, otherwise it is not. By default, the `HttpOnly`
attribute is set.
...
defined in the object is what is used. In the actual `Set-Cookie` response header, only `Max-Age` will be used, so options.cookie.expires will be converted to a `Max-Age` value regardless which one is used in the `options.cookie` object.

##### cookie.path

Specifies the value for the `Path` field in the `Set-Cookie` response header. By default, this is set to `'/'`, which
is the root path of the domain.

##### cookie.sameSite

Specifies the `boolean` or `string` to be the value for the `SameSite` field in the `Set-Cookie` response header.

  - `true` will set the `SameSite` field to `Strict` for strict same site enforcement.
...
Specifies the `boolean` value. If truthy, the `Secure` field in the `Set-Cookie` response header is set, otherwise it is not. By default, the `Secure`
...
##### saveUninitialized (note from me: set to false!!).
