...
export class HermesGraphQLConnector {
  ...
  public static createHermesGraphQLConnector(
    ...
    const user = session && (session['_user'] as IUserProperties | undefined);
    const token =
  ...
  public async resetPassword(
    ...
    if (rstToken === undefined) {
      errors.push(
        new AuthenticationError('token-not-found', 'this token wasnt found')
      );
    }
  ...
  public createUser(
    ...
    if (!this.mustAuthenticate()) {
      // Cant continue
      errors.push(
        new AuthenticationError('user-logged-in', 'User must log out first')
      );
    ...
    if (email === '') {
      errors.push(
        new AuthenticationError('no-email', 'user should provide an email')
      );
    }

    if (password === '') {
      errors.push(
        new AuthenticationError('no-password', 'user should provide a password')
      );
    }
    ...
    if (findName) {
      errors.push(
        new AuthenticationError('username-exist', 'username already in use')
      );
    }
  ...
  public mustAuthenticate(): boolean {
    return (
      this.isAnonymous() || this.isUserBlackListed() || this.hasSessionExpired()
    );
  }
  ...
  public authenticate(
    ...
    if (email === '') {
      errors.push(
        new AuthenticationError('auth-failed', 'user should provide an email')
      );
    }

    if (password === '') {
      errors.push(
        new AuthenticationError('auth-failed', 'user should provide a password')
      );
    }
  ...
  public async requestPasswordReset(_email: string): Promise<IAuthenticationResult> {
    ...
    return this.store
      ...
      .catch(err => {
        const rc: IAuthenticationResult = {
          errors: [
            new AuthenticationError('err-password-reset', err.toString())
          ],
          user: {
            email
          }
